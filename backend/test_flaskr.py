...
from settings import DB_PASSWORD, DB_TEST_NAME, DB_USER
...
class TriviaTestCase(unittest.TestCase):
    ...
    def test_create_new_question(self):
        res = self.client().post('/questions', json={
            "question": "How many legs has a cat",
            "answer": '4',
            "category": 5,
            'difficulty': 3
        })
    ...
    def test_405_if_question_creation_not_allowed(self):
        res = self.client().get('/questions/45', json={
            "question": "How many legs has a cat",
            "answer": 4,
            "category": "art",
            'difficulty': 3
    ...
    def test_get_question_search_without_results(self):
        res = self.client().post('/questions', json={'searchTerm': 'qwerte'})
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 200)
        self.assertEqual(data['success'], True)
    ...
    def test_take_quiz_failed(self):
        res = self.client().post('/quizzes', json={
            'previous_questions': [4],
            'quiz_category': {
                'type': 'Art',
